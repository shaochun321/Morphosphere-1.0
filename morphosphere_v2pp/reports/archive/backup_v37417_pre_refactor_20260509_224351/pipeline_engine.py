...
def write_hypotheses(conn, run_id, adapter, k, cells):
    ...
    for typ, off in [("P_candidate", 0), ("R_candidate", 2)]:
        ...
        if typ.startswith("P") and transport_ok and masking_ok and k >= 3:
            cur_node = "P_frozen"
...
def write_legacy_observable_layer(conn, run_id, adapter, k, cells, hyps):
    ...
    for ph in p_hyps:
        conn.execute(
            ...
            (jid("pb"), os_id, "core", jdump(member_uids[:5]), 0.6+0.02*k, 0.0, f"oa_{adapter.adapter_name}_{k}"))
    ...
    for rh in r_hyps:
        conn.execute(
            ...
            (jid("rb"), os_id, "margin", "counter_structure", "xi_boundary", jdump(["masking_pass", "replay_pass"])))
...
def _compute_bottom_motion_scores(conn, run_id, adapter_name, k, total_windows):
    """Bottom-motion view: support drift, kernel/bandwidth change."""
    ...
    if n == 0:
        return {"p_core": 0.3, "p_band": 0.2, "r_core": 0.1, "r_band": 0.1,
            ...
            "motion_velocity": 0, "fit_score": 0.5}
...
def run_triview_prx_round(conn, run_id, round_number, adapters, windows,
    ...
    for adapter in adapters:
        ...
        for k in range(1, windows):
            ...
            total_xin_start += rho.get("x_true", 0) * 0.1  # proxy start mass
            total_xin_end += rho.get("x_true", 0)
            total_absorbed_p += rho.get("p_band", 0) * 0.05
            total_resolved_r += rho.get("r_band", 0) * 0.03

    # 10. Xin ledger conservation
    x_dissipated = total_xin_start * 0.1
    x_heat_bath = total_xin_start * 0.05
    x_inflow = total_xin_end * 1.2  # proxy inflow
    conservation_gap = abs(
        (total_xin_start + x_inflow) -
        (total_xin_end + total_absorbed_p + total_resolved_r + x_dissipated + x_heat_bath))

    # Count Xin categories
    ...
    chi_x = conservation_gap / max(total_xin_end, 0.01)
    ...
    conn.execute(
        ...
        (jid("xlc"), run_id, round_id,
         total_xin_start, x_inflow, total_absorbed_p, total_resolved_r,
         x_dissipated, x_heat_bath, total_xin_end, conservation_gap, chi_x,
