#!/usr/bin/env python3
"""Morphosphere v37.4.12 batch4 — Memory + Generalization fixes.
- Real transport support → unblock maturity gate → P_frozen memory
- Signal normalization + cross-domain transport → generalization
"""
...
REPORT_DIR = ROOT / "v37.4.12_20260508_batch4_reports"
REPORT_DIR.mkdir(exist_ok=True)
DB_PATH = ROOT / "v37412_20260508_batch4.db"
...
def main():
    ...
    print(f"=== Morphosphere v37.4.12 batch4 (Memory + Generalization) ===")
    ...
    run_id = f"v37412_batch4_{uuid.uuid4().hex[:8]}"
    created = now()
    conn.execute(
        ...
        (run_id,"v37.4.12","v37.4.12","dual_source_v37412",f"full_chain_{CELLS}",
         CELLS*2, WINDOWS, created, "v37.4.12 batch4: memory+generalization",
         CELLS*2, CELLS*2*WINDOWS, eng.jdump({"sources":2,"cells_per_source":CELLS,"focus":"memory_generalization"})))
    ...
    conn.execute("INSERT INTO v367_release_gate (gate_id,run_id,v3671_anchor_pass,v3672_guard_pass,v3673_quarantine_pass,v3674_rmi_pass,legacy_db_mutated,online_native_claimed,overall_verdict,release_notes,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
        (jid("rg"), run_id, 1, 1, 1, 1, 0, 0, "PASS", "v37.4.12 batch4", now()))
    ...
    frozen_count = node_dist.get("P_frozen", 0)
    print(f"  >>> P_frozen hypotheses: {frozen_count}")
    ...
    # Console summary
    print(f"\n{'='*70}")
    print(f"PIPELINE COMPLETE -- v37.4.12_20260508_batch4")
    ...
    print(f"P_frozen: {frozen_count} hypotheses  |  Maturity gates passed: {gate_dist.get('pass',0)}/{sum(gate_dist.values())}")

    # Acceptance gate
    checks = {
        "db_integrity": db_ok == "ok",
        "chain_completeness": integrity.get("all_pass", False),
        "P_frozen_exists": frozen_count > 0,
    ...
    print(f"\n{'='*70}")
    print("ACCEPTANCE GATE (9 checks):")
    all_pass = True
    ...
    # Save reports
    full_report = {
        "meta": {"version":"v37.4.12","batch":"batch4","run_id":run_id,
                 "elapsed_s":round(elapsed,2),"db_size_kb":round(DB_PATH.stat().st_size/1024,1)},
        "memory": {"P_frozen":frozen_count, "maturity_pass":gate_dist.get("pass",0), "node_distribution":node_dist},
    ...
    with open(REPORT_DIR / "v37412_batch4_full_report.json", "w", encoding="utf-8") as f:
